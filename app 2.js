...
function loop(now) {
  ...
  const detections = isHardwareConnected ? liveDetections : detectAll();
  const lidar = isHardwareConnected ? (liveLidar || { distM: null }) : lidarReading();
...
async function connectSerial() {
  ...
  try {
    ...
    await port.open({ baudRate: 115200 }); // standard ESP32 baud
    
    isHardwareConnected = true;
    const hwStatus = document.getElementById('hwStatusPill');
    if (hwStatus) {
      hwStatus.textContent = 'ONLINE';
    ...
    while (true) {
      ...
      if (value) {
        try {
          const pkt = JSON.parse(value);
          if (pkt.lidarDistM !== undefined) {
            liveLidar = { distM: pkt.lidarDistM, hit: null, raw: pkt.lidarDistM, xRelM: 0, corridorHit: true };
          }
          if (pkt.detections) {
            liveDetections = pkt.detections.map(d => ({
              id: d.id || Math.random(),
              prop: { type: d.class ? d.class.toLowerCase() : 'car', xM: d.xRelM, yM: d.yRelM },
              class: d.class || 'CAR',
              color: CLASSES[d.class ? d.class.toLowerCase() : 'car']?.color || '#22d3ee',
              distM: d.distM,
              xRelM: d.xRelM,
              yRelM: d.yRelM,
              conf: d.conf || 1.0
          ...
          if (pkt.speedMps !== undefined) {
             EGO.speedMps = pkt.speedMps;
          }
        } catch (e) {
          // ignore parse errors or malformed json lines
        }
      }
    }
  } catch (err) {
...
// Attach event listener immediately if DOM is ready, or wait for it
